...
export const checkReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case CLEAR_CHECKED_PRODUCT:
            state.noForSelectName.push(state.selectName.id)
            let checkListName = state.NewNames.slice()
            checkListName.find(item => {
                if (item.id === state.selectName.id) {
                    item.isSelectedName = false
                    item.noForSelectName = true
                }
            })
        ...
        case CLEAR_ALL_CHECKED_PRODUCT:
            ...
            for (let i = 0; i < checkNewListName.length; i++) {
                ...
                checkNewListName[i].noForSelectName = false
            }
            ...
            return {
                ...
                selectArt: null,
                selectName: null,
                NewNames: checkNewListName,
                noForSelectName: [],
                newArts: checkNewListNameArt
            }
        ...
        case FETCH_NAME_RESIVED:
            if (action.data.length < 1) {
                message = 'Таблица "ext_Products_forManual" пуста'
            } else {
                message = ''
            }
        ...
        case UPDATE_NEW_PRODUCT:
            let updateNewProduct = state.NewNames.slice()
            updateNewProduct = updateNewProduct.filter(item =>!action.value.includes(item.id))
            return {...state, NewNames: updateNewProduct}
        ...
        case SET_SELECTED_NAME: {
            ...
            if (state.noForSelectName.includes(action.post.id)) {
                message = 'позиция добавлена к сравнению'
                return {...state, NewNames: list, selectName: null, modalMessage: message}
            }
